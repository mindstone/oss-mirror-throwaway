---
description: "CHIEF_ENGINEER2 project overrides for Mindstone Rebel — worktree setup, droid mapping, validation, workflow policies"
...
# Project Overrides — Mindstone Rebel (CHIEF_ENGINEER2)

> **CRITICAL:** This file is read by AI agents at the start of every CHIEF_ENGINEER2 task. It provides Rebel-specific configuration that the shared workflows reference but do not hardcode.
>
> **Companion to [PROJECT_OVERRIDES.md](./PROJECT_OVERRIDES.md)** (the v1 / non-CE2 overrides). v2 deviates from v1 in folder-structure model, terminology (Activities × Specialties), and a few other points — those deltas live here.
>
> **Model routing** is *not* in this file — see [`coding-agent-instructions/workflows/CHIEF_ENGINEER2/MODEL_ROSTER.md`](../../coding-agent-instructions/workflows/CHIEF_ENGINEER2/MODEL_ROSTER.md) for which model class to use for which Activity. The `## Available Droids` table below maps model classes to the actual droid names to dispatch to.
...
**Orchestration:** [CHIEF_ENGINEER2](../../coding-agent-instructions/workflows/CHIEF_ENGINEER2/CHIEF_ENGINEER2.md)
...
The integration-sync command CHIEF_ENGINEER2 §8.6 refers to is [`.factory/commands/git-safe-sync-and-push.md`](../../.factory/commands/git-safe-sync-and-push.md) — the full procedure, including safety rules, escalation triggers, and worktree preflight. It merges `origin/dev`, advances submodules, and pushes directly to `dev` (the direct-push worktree model above). Run it at the §8.6 safe boundaries.
...
See [CHIEF_ENGINEER2 § Review Architecture](../../coding-agent-instructions/workflows/CHIEF_ENGINEER2/CHIEF_ENGINEER2.md#4-review-architecture) for the canonical intensity table and specialist selection guidelines.
...
- **CHIEF_ENGINEER2 Phase 0:** write the CE2 boundary map with `npx tsx scripts/generate-impact-map.ts --output <planning-folder>/generated/boundary-map.json`, then run `npx tsx scripts/generate-boundary-checklist.ts <planning-folder>` to create `BOUNDARY_CHECKLIST.md`.
...
- **Planning docs:** `docs/plans/YYMMDD_<task>/` (v2 planning folder per [CHIEF_ENGINEER2 §6](../../coding-agent-instructions/workflows/CHIEF_ENGINEER2/CHIEF_ENGINEER2.md#6-planning-folder))
...
1. **Determine branch (drives expected evidence):**
   ...
   |---|---|
   | Full visual change recommendation (Chief Designer in CHIEF_ENGINEER2 + implementation completed) | BEFORE + AFTER in both themes (4 total). Custom-accent theme-cycling unavailable: current theme only (2 total). |
...
This is the CHIEF_ENGINEER2 / Chief Bugfixer hook into the shared [CHIEF_DESIGNER](../../coding-agent-instructions/workflows/CHIEF_DESIGNER.md) workflow. Rebel-specific grounding, evidence rules, output addenda, and DSR handoff expectations live in [Chief Designer Overrides](#chief-designer-overrides).

- **When required:** If a CHIEF_ENGINEER2 or CHIEF_BUGFIXER task includes material UI/UX or product-design judgment, invoke the `chief-designer` droid before finalizing the plan or fix approach.
...
- **Wired into the workflow at:**
  - **CHIEF_ENGINEER2:** Phase 0 Design Judgment detection (logged) and Phase 2 critique (invoked alongside reviewers and specialists). See [CHIEF_ENGINEER2.md § Phase 0](../../coding-agent-instructions/workflows/CHIEF_ENGINEER2/CHIEF_ENGINEER2.md#phase-0--intake--setup) and [§ Phase 2 Critique](../../coding-agent-instructions/workflows/CHIEF_ENGINEER2/CHIEF_ENGINEER2.md#phase-2--planning).
...
- This file is the single source of truth for Rebel-specific CHIEF_ENGINEER2 configuration.
